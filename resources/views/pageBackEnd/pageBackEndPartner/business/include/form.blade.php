<div class="row mb-2">
    ...
    <div class="col-md-6 mb-2">
        <div class="form-group">
            ...
            <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror"
                value="{{ isset($business) ? $business->name : old('name') }}" placeholder="{{ __('Company Name') }}"
                required />
            @error('name')
                <span class="text-danger">
                    {{ $message }}
    ...
    <input type="hidden" name="partner_id" id="partner_id"
        class="form-control @error('partner_id') is-invalid @enderror" value="{{ Session::get('id-partner') }}"
        required readonly />
    @error('partner_id')
        <span class="text-danger">
            {{ $message }}
    ...
    <div class="col-md-6 mb-2">
        <div class="form-group">
            <label for="manufacture">{{ __('Manufacture Address') }}</label>
            <textarea name="manufacture" id="manufacture" class="form-control @error('manufacture') is-invalid @enderror"
                placeholder="{{ __('Manufacture Address') }}" required>{{ isset($business) ? $business->manufacture : old('manufacture') }}</textarea>
            @error('manufacture')
                <span class="text-danger">
                    {{ $message }}
                </span>
        ...
        <div class="col-md-12 mb-2">
            <div class="row">
                ...
                <div class="col-md-4 text-center">
                    ...
                    @else
                        <img src="{{ asset('storage/uploads/logos/' . $business->logo) }}" alt="Logo"
